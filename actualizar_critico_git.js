async function actualizar_existencias_git(data_actual) {
  ...
  const FILE_PATH = "base_critico_" + localStorage.getItem("pais") + ".json";
  ...
  try {
    await actualizarArchivo();
     document.getElementById("actualizacion").innerHTML = "Archivo actualizado con éxito. ";
  document.getElementById("actualizacion").style.background = "#2da849a3";
  } catch (error) {
    document.getElementById("actualizacion").innerHTML = "Error al actualizar el archivo en Base. " + error.toUpperCase();
