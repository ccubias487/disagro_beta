...
document.getElementById("listado").innerHTML="LISTADO DE ORDENES "+ localStorage.getItem("ordenes_mostrar")
...
const jsondata = actividades.sort((a, b) => {
  ...
  return fechaA - fechaB;
    ...
    for (let i in jsondata) {
      ...
      function iniciar_orden(orden) {
      generar_orden_finalizadaPDF(orden)
      }
      ...
      if (jsondata[i].ESTATUS == localStorage.getItem("ordenes_mostrar")) {
        console.log(jsondata[i].TECNICO)
        let tablaRef = document.getElementById("tabla_requisiciones");
        let filaRef = tablaRef.insertRow(-1);
        if(localStorage.getItem("ordenes_mostrar")=="FINALIZADA"){
        filaRef.onclick =  () => iniciar_orden(jsondata[i].ORDEN);
        }
        const prioridadColor = {
          "A": "rgb(202, 80, 80)",
          "B": "rgb(223, 236, 89)",
